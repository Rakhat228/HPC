...
void threadFunc(Scene &scene, ViewPlane &viewPlane, Image &image, int sizeX, int sizeY, int numOfSamples, int init, int end) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < numberOfThreads; i++) {
        ...
        thread thr(threadFunc, ref(scene), ref(viewPlane), ref(image), 
            viewPlaneResolutionX, viewPlaneResolutionY, numOfSamples, init, end);
